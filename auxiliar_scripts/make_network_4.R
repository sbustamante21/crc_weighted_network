...
counts <- read.table("../input_data/mirna_counts.txt", header = TRUE, row.names = 1, sep="\t")
metadata <- read.table("../input_data/mirna_metadata.csv", header = TRUE, row.names = 1, sep=";")
...
svg("../output/scale_independence_and_mean_connectivity.svg", width=8, height=6)
...
softPower <- 9 # Can change depending on the network
...
print("TOMplot")
png("../output/TOMplot.png", width=8, height=6, units="in", res=300)
